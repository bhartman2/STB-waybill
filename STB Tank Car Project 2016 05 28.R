...
#this does not work for 2012, 2013, 2014 
# ??because the format of the timestamp is wrong?
...
# # does not work right, the one below is does not work right either.
# q_mycommod_tank <- paste("SELECT `car_ownership_code`, `AAR_car_type`, `ICC_car_type` 'ICCtype', `commodity_code` 'commodity', `freight_rev`, `transit_charges`, `misc_charges`, 
# `no_carloads` 'carloads', `expand_carloads` 'xcarloads'
# FROM ",tablename, 
# " WHERE `ICC_car_type` between 50 and 51 
# and LEFT(`commodity_code`,2) in (13,20,28,29) 
# ORDER BY `commodity_code` 
# LIMIT 1000 OFFSET 0",
# sep="")
# sqlQuery(stecher,"SET SQL_BIG_SELECTS=1")
# q_mycommod_tank
# (tank_recs <- sqlQuery(stecher,q_mycommod_tank))
# 
# ###################################
# 
# # does not work right, the one below is does not work right either.
# q_commod_tank <- paste("SELECT distinct w.`commodity_code` 'commodity',
# left(c.`comdesc`,40) 'description'
# FROM ",tablename," w, spcom c
# WHERE w.`ICC_car_type` between 50 and 51
# and left(w.`commodity_code`,2) in (13,20,28,29)
# /* and w.`commodity_code` = c.`comcode` */
# GROUP BY w.`commodity_code`
# ORDER BY w.`commodity_code` asc",
#     sep="")
# q_commod_tank
# sqlQuery(stecher,"SET SQL_BIG_SELECTS=1")
# (tank_commod <- sqlQuery(stecher,q_commod_tank))
# 
# # tank_commod[tank_commod$commodity>=29000 & tank_commod$commodity<30000,]
# # tank_commod
###################################
